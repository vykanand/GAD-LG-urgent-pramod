...
ipcMain.handle('print', async (event, options = {}) => {
  try {
    return await new Promise((resolve) => {
      event.sender.print(Object.assign({ printBackground: true }, options), (success, failureReason) => {
        if (success) resolve({ success: true });
        else resolve({ success: false, error: failureReason || 'Unknown' });
      });
    });
  } catch (err) {
    return { success: false, error: err.message };
...
ipcMain.handle('print-preview', async (event, options = {}) => {
  try {
    const pdfOptions = Object.assign({ printBackground: true, marginsType: 1 }, options);
    const data = await event.sender.printToPDF(pdfOptions);
    const tmpPath = path.join(app.getPath('temp'), `app-print-preview-${Date.now()}.pdf`);
    fsSync.writeFileSync(tmpPath, data);
    // Open the generated PDF in the system default viewer so user can preview and print
